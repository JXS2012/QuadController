...
void VisualBird::visualCallback(const std_msgs::Float32MultiArray msg)
{
  ...
  else
  {
    ...
    pcl::PointXYZ drifterror,temp;
    //drifterror = scalarProduct(120./240*(eye->getCurrentPoint().z-eye->getOriginPoint().z)/300.,mid);
    drifterror = scalarProduct(140./240,mid);

    temp = drifterror;

    temp.x = drifterror.x * cos(M_PI*3/4) - drifterror.y * sin(M_PI*3/4);
    temp.y = drifterror.x * sin(M_PI*3/4) + drifterror.y * cos(M_PI*3/4);

    drifterror = temp;
    
    if (start_visual)
    {
      pcl::PointXYZ tempTarget;	
      tempTarget.x = eye->getCurrentPoint().x + drifterror.x * cos(eye->getPsi()) - drifterror.y * sin(eye->getPsi());
      tempTarget.y = eye->getCurrentPoint().y + drifterror.x * sin(eye->getPsi()) + drifterror.y * cos(eye->getPsi());
      tempTarget.z = targetBound.z;
      
      if (norm(vectorMinus(targetBound, tempTarget)) > 500)
      {
	     ROS_INFO("Out range");
        targetPoint = eye->getCurrentPoint();
        targetPoint.z = targetBound.z;
        return;
      }

      if (abs(tempTarget.x) > 10000){	
     	ROS_INFO("Target Point %.3f %.3f %.3f",targetPoint.x,targetPoint.y,targetPoint.z);   
      ROS_INFO("Drift %.3f %.3f", drifterror.x, drifterror.y);
...
void VisualBird::checkStartPoint()
{
  ...
  if (vectorMinus(eye->getCurrentPoint(),targetPoint).z<50 && !reachStartPoint)
    {
...
void VisualBird::writeLog(int fileNo)
{
  ...
  if (pFile!=NULL)
	...
	fprintf(pFile, "x %.3f y %.3f z %.3f vx %.3f vy %.3f vz %.3f ax %.3f ay %.3f az %.3f roll %.6f pitch %.6f thrust %.6f targetX %.3f targetY %.3f targetZ %.3f\n", eye->getLogPos(i).x, eye->getLogPos(i).y, eye->getLogPos(i).z, eye->getLogVel(i).x,eye->getLogVel(i).y,eye->getLogVel(i).z, eye->getLogAcc(i).x, eye->getLogAcc(i).y, eye->getLogAcc(i).z, dummy->getLogRoll(i), dummy->getLogPitch(i), dummy->getLogThrust(i), visualTarget[i].x, visualTarget[i].y, visualTarget[i].z);
    }
  fclose(pFile);
