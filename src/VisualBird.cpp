...
void VisualBird::drive()
{
  ...
  /*
  if (norm(vectorMinus(eye->getCurrentPoint(), shiftOrigin)) < 150 && !landing && !hover)
    {
      switchHover();
      targetPoint = shiftOrigin;
    }
  */
  ...
  if (reachStartPoint && start_visual && !landing)
    {
  ...
  //ROS_INFO("targetPoint %f %f %f", msg.point.x, msg.point.y, msg.point.z);
