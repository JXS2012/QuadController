...
class VisualBird
{
 private:
  ...
  //virtual target
  bool circleStart;
  double initiateCircleTime;
  pcl::PointXYZ targetCircleCenter, targetVel, targetAcc;
  double currTime, radius, pi, omega, currTheta; 
  std::vector<pcl::PointXYZ> targetPosLog,targetVelLog,targetAccLog;

  std::vector<pcl::PointXYZ> vel_ref,acc_ref;
